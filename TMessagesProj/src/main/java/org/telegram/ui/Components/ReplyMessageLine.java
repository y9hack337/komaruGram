        ...
        if (messageObject == null) {
            ...
            if (messageObject.overrideLinkColor >= 0) {
                ...
                if (dialogId < 0) {
                    ...
                    if (chat != null) {
                        colorId = komarugramAppearanceConfig.INSTANCE.getReplyCustomColors() ? ChatObject.getColorId(currentChat) : 0;
                    }
                    if (type == TYPE_LINK) {
                        emojiDocumentId = komarugramAppearanceConfig.INSTANCE.getReplyBackgroundEmoji() ? ChatObject.getEmojiId(chat) : 0;
                    }
                } else {
                    TLRPC.User user = MessagesController.getInstance(messageObject.currentAccount).getUser(dialogId);
                    if (user != null) {
                        colorId = komarugramAppearanceConfig.INSTANCE.getReplyCustomColors() ? UserObject.getColorId(user) : 0;
                    }
                    if (type == TYPE_LINK) {
                        emojiDocumentId = komarugramAppearanceConfig.INSTANCE.getReplyBackgroundEmoji() ? UserObject.getEmojiId(user) : 0;
                    }
                }
                ...
                colorId = komarugramAppearanceConfig.INSTANCE.getReplyCustomColors() ? UserObject.getColorId(currentUser) : 0;
                if (type == TYPE_LINK) {
                    emojiDocumentId = komarugramAppearanceConfig.INSTANCE.getReplyBackgroundEmoji() ? UserObject.getEmojiId(currentUser) : 0;
                }
                ...
                if (currentChat.signature_profiles) {
                    ...
                    if (did >= 0) {
                        ...
                        colorId = komarugramAppearanceConfig.INSTANCE.getReplyCustomColors() ? ChatObject.getColorId(chat) : 0;
                        if (type == TYPE_LINK) {
                            emojiDocumentId = komarugramAppearanceConfig.INSTANCE.getReplyBackgroundEmoji() ? ChatObject.getEmojiId(chat) : 0;
                        }
                    }
                    ...
                    colorId = komarugramAppearanceConfig.INSTANCE.getReplyCustomColors() ? ChatObject.getColorId(currentChat) : 0;
                    if (type == TYPE_LINK) {
                        emojiDocumentId = komarugramAppearanceConfig.INSTANCE.getReplyBackgroundEmoji() ? ChatObject.getEmojiId(currentChat) : 0;
                    }
                }
            } else {
                colorId = 0;
            ...
            backgroundColor = komarugramAppearanceConfig.INSTANCE.getReplyBackground() ? Theme.multAlpha(color1, 0.10f) : Color.TRANSPARENT;
            nameColor = color1;
        } else {
            hasColor2 = false;
            hasColor3 = false;
