...
public class ProfileSearchCell extends BaseCell implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    public void buildLayout() {
        ...
        if (encryptedChat != null) {
            ...
            drawPremium = !savedMessages && MessagesController.getInstance(currentAccount).isPremiumUser(user) && !komarugramAppearanceConfig.INSTANCE.getDisablePremiumStatuses();
    ...
    public void updateStatus(boolean verified, TLRPC.User user, TLRPC.Chat chat, boolean animated) {
        ...
        } else if (user != null && !savedMessages && DialogObject.getEmojiStatusDocumentId(user.emoji_status) !=0 && !komarugramAppearanceConfig.INSTANCE.getDisablePremiumStatuses()) {
        ...
        } else if (chat != null && !savedMessages && DialogObject.getEmojiStatusDocumentId(chat.emoji_status) != 0 && !komarugramAppearanceConfig.INSTANCE.getDisablePremiumStatuses()) {
            statusDrawable.set(DialogObject.getEmojiStatusDocumentId(chat.emoji_status), animated);
            statusDrawable.setColor(Theme.getColor(Theme.key_chats_verifiedBackground, resourcesProvider));
        } else if (user != null && !savedMessages && MessagesController.getInstance(currentAccount).isPremiumUser(user) && !komarugramAppearanceConfig.INSTANCE.getDisablePremiumStatuses()) {
