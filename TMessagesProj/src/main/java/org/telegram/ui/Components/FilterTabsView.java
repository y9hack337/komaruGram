...
public class FilterTabsView extends FrameLayout {
    ...
    int tabMode = komarugramAppearanceConfig.INSTANCE.getTabMode();
    ...
    public class Tab {
        ...
        public Tab(int i, String t, String e) {
            id = i;
            title = tabMode != komarugramAppearanceConfig.TAB_TYPE_ICON ? t:"";
            realTitle = t;
            emoticon = e;
        }
        ...
        public boolean setTitle(String newTitle) {
            ...
            title = tabMode != komarugramAppearanceConfig.TAB_TYPE_ICON ? newTitle : "";
            realTitle = newTitle;
            return true;
        }
    }
    ...
    public class TabView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (tabMode != komarugramAppearanceConfig.TAB_TYPE_ICON) {
            ...
            float textX = ((getMeasuredWidth() - tabWidth) / 2f) + currentTab.iconWidth - (tabMode == komarugramAppearanceConfig.TAB_TYPE_MIX ? AndroidUtilities.dp(2) : 0);
            if (animateTextX) {
            ...
            int iconX = 0;
            // TAB ICON
            if (tabMode != komarugramAppearanceConfig.TAB_TYPE_TEXT) {
            ...
            if (animateCounterEnter || counterText != null || showRemove && (isEditing || editingStartAnimationProgress != 0)) {
                ...
                int textSpace = AndroidUtilities.dp(tabMode == komarugramAppearanceConfig.TAB_TYPE_ICON && tabStyle <= komarugramAppearanceConfig.TAB_STYLE_TEXT ? 3 : 6);
    ...
    private final Property<FilterTabsView, Float> COLORS = new AnimationProperties.FloatProperty<FilterTabsView>("animationValue") {
        ...
        public void setValue(FilterTabsView object, float value) {
            ...
            selectorDrawable.setColor(ColorUtils.setAlphaComponent(ColorUtils.blendARGB(color1, color2, value), tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI ? 0x2F : 0xFF));
    ...
    public FilterTabsView(Context context) {
        ...
        listView.setSelectorType(tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI ? 100 : 8);
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (isEditing) {
                if (position != 0 || komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats()) {
                    int side = AndroidUtilities.dp(6);
                    if (tabView.rect.left - side < x && tabView.rect.right + side > x) {
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (child == listView) {
            ...
            if (indicatorWidth != 0 && tabStyle != komarugramAppearanceConfig.TAB_STYLE_TEXT) {
                canvas.save();
                ...
                updateSelector();
                selectorDrawable.setBounds((int) indicatorX - (tabStyle == komarugramAppearanceConfig.TAB_STYLE_VKUI ? AndroidUtilities.dp(8) : tabStyle == komarugramAppearanceConfig.TAB_STYLE_PILLS ? AndroidUtilities.dp(10) : 0), tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI ? height / 2 - AndroidUtilities.dp(15) : height - AndroidUtilities.dpr(4), (int) (indicatorX + indicatorWidth) + (tabStyle == komarugramAppearanceConfig.TAB_STYLE_VKUI ? AndroidUtilities.dp(8) : tabStyle == komarugramAppearanceConfig.TAB_STYLE_PILLS ? AndroidUtilities.dp(10) : 0), tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI ? height / 2 + AndroidUtilities.dp(15) : height);
...
//                    selectorDrawable.setColor(ColorUtils.setAlphaComponent(Theme.getColor(tabLineColorKey), 0));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!tabs.isEmpty()) {
            ...
            if (firstTab != null && !komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats())  {
    ...
    public void checkTabsCounter() {
        ...
        for (int a = 0, N = tabs.size(); a < N; a++) {
            ...
            if (oldWidth != width || invalidated) {
                ...
                requestLayout();
                allTabsWidth = 0;
                if (!komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats()) findDefaultTab().setTitle(LocaleController.getString(R.string.FilterAllChats));
                for (int b = 0; b < N; b++) {
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public void swapElements(int fromIndex, int toIndex) {
            ...
            ArrayList<MessagesController.DialogFilter> filters = MessagesController.getInstance(UserConfig.selectedAccount).getDialogFilters();
            if (komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats()) {
...
//                notifyItemMoved(i, i + 1);
    ...
    public class TouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            if (!komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats() && MessagesController.getInstance(UserConfig.selectedAccount).premiumFeaturesBlocked() && (!isEditing || (viewHolder.getAdapterPosition() == 0 && tabs.get(0).isDefault && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium()))) {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            if (!komarugramAppearanceConfig.INSTANCE.getTabsHideAllChats() && MessagesController.getInstance(UserConfig.selectedAccount).premiumFeaturesBlocked() && ((source.getAdapterPosition() == 0 || target.getAdapterPosition() == 0) && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium())) {
                return false;
            }
    ...
    public void updateSelector() {
        selectorDrawable.setColor(ColorUtils.setAlphaComponent(Theme.getColor(tabLineColorKey), tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI ? 0x2F : 0xFF));
        float rad = AndroidUtilities.dpf2(tabStyle == komarugramAppearanceConfig.TAB_STYLE_VKUI ? 10 : tabStyle == komarugramAppearanceConfig.TAB_STYLE_PILLS ? 30 : 3);
        if (tabStyle == komarugramAppearanceConfig.TAB_STYLE_ROUNDED || tabStyle >= komarugramAppearanceConfig.TAB_STYLE_VKUI) {
