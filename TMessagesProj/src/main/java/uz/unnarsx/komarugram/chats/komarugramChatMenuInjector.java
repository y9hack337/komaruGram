...
public class komarugramChatMenuInjector {
    ...
    public static void injectkomarugramShortcuts(ActionBarMenuItem headerItem, TLRPC.Chat currentChat, TLRPC.User currentUser) {
        boolean isAnyButtonEnabled = komarugramPrivacyConfig.INSTANCE.getAskForPasscodeBeforeOpenChat() || komarugramChatsConfig.INSTANCE.getShortcut_JumpToBegin()
                || komarugramChatsConfig.INSTANCE.getShortcut_DeleteAll() || komarugramChatsConfig.INSTANCE.getShortcut_SavedMessages()
                || komarugramChatsConfig.INSTANCE.getShortcut_Blur() || komarugramChatsConfig.INSTANCE.getShortcut_Browser();

        if (isAnyButtonEnabled) headerItem.lazilyAddColoredGap();

        if (komarugramPrivacyConfig.INSTANCE.getAskForPasscodeBeforeOpenChat() && ChatsPasswordHelper.INSTANCE.getAskPasscodeForChats()) {
            if (
        ...
        if (komarugramChatsConfig.INSTANCE.getShortcut_JumpToBegin())
        ...
        if ((ChatObject.isMegagroup(currentChat) || currentChat != null && !ChatObject.isChannel(currentChat)) && !ChatsHelper2.INSTANCE.isDeleteAllHidden(currentChat)) {
            if (komarugramChatsConfig.INSTANCE.getShortcut_DeleteAll())
    ...
    public static void injectAdminShortcuts(ActionBarMenuItem headerItem, TLRPC.Chat currentChat) {
        ...
        boolean isAnyButtonEnabled = komarugramChatsConfig.INSTANCE.getAdmins_Reactions() || komarugramChatsConfig.INSTANCE.getAdmins_Permissions() || komarugramChatsConfig.INSTANCE.getAdmins_Administrators()
                || komarugramChatsConfig.INSTANCE.getAdmins_Members() || komarugramChatsConfig.INSTANCE.getAdmins_Statistics() || komarugramChatsConfig.INSTANCE.getAdmins_RecentActions();

        if (isAnyButtonEnabled) headerItem.lazilyAddColoredGap();

        if (komarugramChatsConfig.INSTANCE.getAdmins_Reactions() && ChatObject.canChangeChatInfo(currentChat)) {
        ...
        if (komarugramChatsConfig.INSTANCE.getAdmins_Permissions() && !(ChatObject.isChannel(currentChat) && !currentChat.megagroup) && !currentChat.gigagroup) {
        ...
        if (komarugramChatsConfig.INSTANCE.getAdmins_Administrators()) {
        ...
        if (komarugramChatsConfig.INSTANCE.getAdmins_Members()) {
        ...
        if (komarugramChatsConfig.INSTANCE.getAdmins_Statistics() && ChatObject.isBoostSupported(currentChat)) {
        ...
        if (komarugramChatsConfig.INSTANCE.getAdmins_RecentActions()) {
