...
object KomarugramExtras : CoroutineScope by MainScope() {
    ...
    private const val channelUsername = "komarugram"
    @JvmStatic
...
//            MessagesController.getMainSettings(currentAccount).edit().putBoolean("update_channel_follow_skip", true).apply()
        ...
        AndroidUtilities.runOnUIThread {
            ...
            builder.setNeutralButton(getString(R.string.ProfileJoinChannel)) { _, _ ->
                messagesCollector.addUserToChat(channel.id, userConfig.currentUser, 0, null, null, null)
                Browser.openUrl(ctx, "https://t.me/$channelUsername")
            }
...
//            builder.setNegativeButton(getString(R.string.Cancel), null)
    ...
    fun requestReviewFlow(fragment: BaseFragment, context: Context, activity: Activity) {
        ...
        requestReviewFlow.addOnCompleteListener { request ->
            if (!MessagesController.getMainSettings(fragment.currentAccount).getBoolean("is_komarugram_rated", false)) {
                if (request.isSuccessful) {
                    ...
                    flow.addOnCompleteListener {
                        ...
                        MessagesController.getMainSettings(fragment.currentAccount).edit().putBoolean("is_komarugram_rated", true).apply()
                    }
                } else {
